<div>
    <div class="flex justify-end">
        <button class="mx-5 my-4 mb-2 bg-gray-200 py-1 px-2 rounded-md font-sm hover:bg-gray-300" wire:click="$emit('closeModal')">X</button>
    </div>
    <form method="post" action="{{ route('order.addOrder', $order['id']) }}">
        @csrf
    <div class="h-[85vh] mx-auto px-5 bg-white">
            <div class="h-full flex lg:flex-row flex-col-reverse shadow-lg">
                <!-- left section -->
                ...
                <div class="flex flex-col h-full w-full lg:w-2/5">
                    ...
                    <div class="grow px-5 py-4 mt-5 overflow-y-auto shadow-md">
                        @foreach($carts as $index => $cart)
                            ...
                            <input type="hidden" name="order_items[{{$index}}][subtotal]" value="{{ $cart['price'] * $cart['quantity'] }}">
                        @endforeach
                    </div>
                    <!-- end order list -->
                    <!-- totalItems -->
                    <div class="px-5 mt-5">
                        ...
                        <input wire:model="order.order_no" type="hidden" name="order_no">
